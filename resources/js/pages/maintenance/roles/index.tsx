...
// Types
import { Role } from '@/types/maintenance';
...
export default function Roles() {
    ...
    const fetchRoles = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${WebUrl}/api/maintenance/roles`);
    ...
    const openCreateSheet = () => {
        setEditId(null);
        setForm({ role: '', isActive: true });
        setIsSheetOpen(true);
    };

    const openEditSheet = (item: Role) => {
        setEditId(item.role_id);
        setForm({ role: item.role, isActive: !!item.isActive });
        setIsSheetOpen(true);
    };
    ...
    const handleSaveRole = async (e: React.FormEvent) => {
        ...
        if (data.some(item => item.role.toLowerCase() === roleName.toLowerCase() && item.role_id !== editId)) {
            return toast.error(`Role "${roleName}" is already registered.`);
        }

        setSubmitting(true);
        try {
            if (editId) {
                await axios.put(`${WebUrl}/api/maintenance/roles/${editId}`, form);
                toast.success('Role updated successfully');
            } else {
                await axios.post(`${WebUrl}/api/maintenance/roles`, form);
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            {loading ? (
                ...
                <div className="w-full space-y-6 px-4 py-6 md:px-8 lg:px-16">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-6">
                        <div>
                            <h1 className="text-lg font-bold tracking-tight">Role Registry</h1>
                            <p className="text-xs text-muted-foreground">Manage and configure system application roles.</p>
                        </div>
                    ...
                    <div className="rounded-xl border bg-card text-card-foreground shadow-sm overflow-hidden">
                        <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-muted/50 border-b text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                            <div className="col-span-7">Role Details</div>
                        ...
                        {filteredData.length === 0 ? (
                            ...
                            <div className="divide-y">
                                {displayedItems.map((item) => (
                                    <div key={item.role_id} className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-muted/30 transition-colors">
            ...
            <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
                <SheetContent className="sm:max-w-md">
                    <SheetHeader>
                        <SheetTitle>{editId ? 'Edit Role' : 'Register Role'}</SheetTitle>
                        <SheetDescription>
                            {editId ? 'Modify the existing role settings.' : 'Fill in the details to add a new system role.'}
                        </SheetDescription>
                    </SheetHeader>
                    
                    <form onSubmit={handleSaveRole} className="space-y-6 py-8 px-4">
                        ...
                        <SheetFooter className="mt-8">
                            <Button type="submit" disabled={submitting} className="w-full h-11 bg-indigo-600 cursor-pointer text-white">
                                {submitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : (editId ? 'Save Changes' : 'Register Role')}
                            </Button>
                        </SheetFooter>
                    </form>
                </SheetContent>
